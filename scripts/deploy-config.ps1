...
if (-not (Test-Path ".env")) {
    ...
    if (Test-Path ".env.example") {
        ...
        if ($USER_DB_AUTH) {
            ...
            Write-Host "${Yellow}⚠️  Could not auto-generate USER_DB_AUTH - will prompt later${Reset}"
        }
        ...
        if ($R2_KEY_SECRET) {
            ...
            Write-Host "${Yellow}⚠️  Could not auto-generate R2_KEY_SECRET - will prompt later${Reset}"
        }
        ...
        if ($KEYS_AUTH) {
            ...
            Write-Host "${Yellow}⚠️  Could not auto-generate KEYS_AUTH - will prompt later${Reset}"
        }
        
        Write-Host "${Green}🔐 Authentication secrets auto-generated! Continuing with configuration...${Reset}"
        Write-Host ""
    } else {
...
if (-not $env:USER_DB_AUTH -or $env:USER_DB_AUTH -eq "your_custom_user_db_auth_token_here") {
    try {
        $USER_DB_AUTH = & openssl rand -hex 32 2>$null
        if (-not $USER_DB_AUTH) { throw "OpenSSL failed" }
    } catch {
    ...
    if ($USER_DB_AUTH) {
        (Get-Content ".env") -replace "your_custom_user_db_auth_token_here", $USER_DB_AUTH -replace "^USER_DB_AUTH=.*", "USER_DB_AUTH=$USER_DB_AUTH" | Set-Content ".env"
        Write-Host "${Green}✅ USER_DB_AUTH auto-generated${Reset}"
        $env:USER_DB_AUTH = $USER_DB_AUTH
    } else {
        Write-Host "${Yellow}⚠️  Could not auto-generate USER_DB_AUTH - will prompt later${Reset}"
    }
}
...
if (-not $env:R2_KEY_SECRET -or $env:R2_KEY_SECRET -eq "your_custom_r2_secret_here") {
    try {
        $R2_KEY_SECRET = & openssl rand -hex 32 2>$null
        if (-not $R2_KEY_SECRET) { throw "OpenSSL failed" }
    } catch {
    ...
    if ($R2_KEY_SECRET) {
        (Get-Content ".env") -replace "your_custom_r2_secret_here", $R2_KEY_SECRET -replace "^R2_KEY_SECRET=.*", "R2_KEY_SECRET=$R2_KEY_SECRET" | Set-Content ".env"
        Write-Host "${Green}✅ R2_KEY_SECRET auto-generated${Reset}"
        $env:R2_KEY_SECRET = $R2_KEY_SECRET
    } else {
        Write-Host "${Yellow}⚠️  Could not auto-generate R2_KEY_SECRET - will prompt later${Reset}"
    }
}
...
if (-not $env:KEYS_AUTH -or $env:KEYS_AUTH -eq "your_custom_keys_auth_token_here") {
    try {
        $KEYS_AUTH = & openssl rand -hex 32 2>$null
        if (-not $KEYS_AUTH) { throw "OpenSSL failed" }
    } catch {
    ...
    if ($KEYS_AUTH) {
        (Get-Content ".env") -replace "your_custom_keys_auth_token_here", $KEYS_AUTH -replace "^KEYS_AUTH=.*", "KEYS_AUTH=$KEYS_AUTH" | Set-Content ".env"
        Write-Host "${Green}✅ KEYS_AUTH auto-generated${Reset}"
        $env:KEYS_AUTH = $KEYS_AUTH
    } else {
        Write-Host "${Yellow}⚠️  Could not auto-generate KEYS_AUTH - will prompt later${Reset}"
    }
}
...
Write-Host "${Green}🔐 Authentication secrets generation complete!${Reset}"
