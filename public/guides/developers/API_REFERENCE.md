...
8. [Audit Worker API](#audit-worker-api)
   - [Endpoints](#endpoints-5)
     - [Store Audit Entry](#store-audit-entry)
     - [Get User Audit Entries](#get-user-audit-entries)
9. [Turnstile Worker API](#turnstile-worker-api)
...
Striae's backend consists of seven Cloudflare Workers that provide specialized API services. All APIs use RESTful conventions and return JSON responses.
...
- **Audit Worker**: Uses `R2_KEY_SECRET` environment variable
...
**Base URL**: `{AUDIT_WORKER_URL}`

**Authentication**: `X-Custom-Auth-Key` header with API key from Keys Worker

**Storage**: Cloudflare R2 bucket (`STRIAE_AUDIT` - Dedicated audit bucket)

**Environment Variables Required**:
- `R2_KEY_SECRET` - R2 bucket authentication for accessing STRIAE_AUDIT bucket
...
**Parameters**:
- `userId` (path): User identifier for audit trail organization
...
```json
{
  ...
  "result": "success",
  "caseNumber": "CASE-2025-001",
  "fileName": "case-data.json",
  "fileId": "file-123",
  "fileSize": 15240,
  "fileMimeType": "application/json",
  "details": {
    "caseName": "Test Case",
    "caseDescription": "Comparison examination",
    "fileCount": 3,
    "annotationCount": 5,
...
| `action` | string | Yes | What action was performed (case-create, file-upload, export, etc.) |
| `workflowPhase` | string | Yes | Context phase (casework, case-export, case-import, confirmation, user-management) |
| `result` | string | Yes | Result status (success, failure, warning, blocked, pending) |
| `caseNumber` | string | No | Associated case number (omit for non-case actions) |
| `fileName` | string | No | Associated file name |
| `fileId` | string | No | Unique file identifier |
| `fileSize` | number | No | File size in bytes |
| `fileMimeType` | string | No | MIME type of file |
| `details` | object | No | Action-specific metadata and performance metrics |
...
- `action` (optional): Filter by specific action (case-create, file-upload, export, etc.)
- `result` (optional): Filter by result type (success, failure, warning, blocked)
...
**Example**:
```http
GET /aDzwq3G6IBVRJVCEFijdg7B0fwq2/?startDate=2025-09-01&endDate=2025-09-30&action=case-create&limit=50
```
...
- Immutable entries: Once created, audit entries cannot be modified or deleted
